...
std::tuple<int, int> Environment::pos_to_index(const Vec2 &pos) {
    auto int_pos = (pos - top_left_position.cast<float>() +
                    Vec2(offset_padding.left, offset_padding.top)).array().round().cast<int>();
...
void Environment::resolve_collisions(const std::vector<Vec2> &closests) {
    ...
    for (int i = 0; i < yaals.size(); i++) {
        ...
        float size = (float) yaals[i].genome.size /
                     2.f; // TODO : when the sizes are not the same, put Yaals in quadtree, not only positions
...
void Environment::resolve_border_collisions() {
    for (auto &yaal: yaals) {
        float size = (float) yaal.genome.size / 2.f;
        yaal.position = yaal.position.cwiseMax(Vec2(size, size)).cwiseMin(
