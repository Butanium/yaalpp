...
Stream::Stream(const char *filename, int fps, Size size, int workers_row, int workers_col, bool z_only_writer,
               MPI_Comm comm)
        : filename(filename), size(size), workers_row(workers_row), workers_col(workers_col),
          z_only_writer(z_only_writer), comm(comm) {
    int rank, comm_size;
...
void Stream::append_frame(Mat *frame, const char *frame_filename) {
    Mat resized;
    ...
    MPI_Gatherv(resized.data, resized.total() * resized.elemSize(), MPI_UNSIGNED_CHAR, this->buffer, this->recvcounts,
                this->displs, MPI_UNSIGNED_CHAR, 0, this->comm);

    if (this->writer != nullptr) {

#pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < comm_size - (this->z_only_writer ? 1 : 0); i++) {
            Mat worker_frame(this->worker_size, CV_8UC3,
                             this->buffer + (i * this->worker_size.area() * this->img_buffer->elemSize()));
            Rect worker_rect((i % this->workers_row) * this->worker_size.width,
                             (i / this->workers_row) * this->worker_size.height, this->worker_size.width,
                             this->worker_size.height);
        ...
        if (frame_filename != nullptr) {
            imwrite(frame_filename, *this->img_buffer);
        }
    }
}

void Stream::append_frame(Tensor<float, 3> &frame, const char *frame_filename) {
    Mat *image = eigen2cv(frame);
    this->append_frame(image, frame_filename);
    delete image;
}
